...
public class SwipeTest : MonoBehaviour
{
    ...
    private void SwipeGestureCallback(GestureRecognizer gesture)
    {
        ...
        if (gesture.State == GestureRecognizerState.Ended)
        {
            ...
            Debug.Log(string.Format("Swiped from {0},{1} to {2},{3}; velocity: {4}, {5}", gesture.StartFocusX, gesture.StartFocusY, gesture.FocusX, gesture.FocusY, swipeGesture.VelocityX, swipeGesture.VelocityY));
        }
    }
    ...
    private void CreateFlyingSprite(Vector2 startPosition, Vector2 endPosition)
    {
        ...
        // プレハブからゲームオブジェクトを生成
        GameObject spriteObj = Instantiate(spritePrefab, worldStartpos, Quaternion.identity);
        // スワイプの方向に適当な力を加える
